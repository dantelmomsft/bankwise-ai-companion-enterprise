...
export default function BillManagement() {
  ...
  const totalPending = billsData.filter(bill => bill.status === 'pending').reduce((sum, bill) => sum + bill.amount, 0);
  const totalOverdue = billsData.filter(bill => bill.status === 'overdue').reduce((sum, bill) => sum + bill.amount, 0);
  const totalPaid = billsData.filter(bill => bill.status === 'paid').reduce((sum, bill) => sum + bill.amount, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="bg-card/50 backdrop-blur border-border/50">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">1 bill paid</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card/50 backdrop-blur border-border/50">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Average monthly bills</p>
          </CardContent>
        </Card>
      </div>

      {/* Bills List and Chart */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Bills List */}
        <Card className="bg-card/50 backdrop-blur border-border/50">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-foreground">Current Bills</CardTitle>
          ...
          <CardContent>
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {filteredBills.map((bill) => (
                <div key={bill.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
        ...
        <Card className="bg-card/50 backdrop-blur border-border/50">
          <CardHeader>
            <CardTitle className="text-foreground">Monthly Bill Trends</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={350}>
              <BarChart data={monthlyBillTrends}>
